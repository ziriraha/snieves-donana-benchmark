...
from app.utils.constants import CSV_DIRECTORY, SPECIES_NAMES, CUSTOM_DIRECTORY, ERROR_MESSAGES
...
def get_species_images(species_name):
    ...
    if len(data) == 0:
        logger.error(f'No data found for {species_name} at {date} max {num_files}')
        return jsonify({'error': ERROR_MESSAGES['no_data']}), 404

    logger.info(f'SPECIES_REQUEST: {species_name} at {date} max {num_files} ({zip_name}), total of {len(data)} images.')
